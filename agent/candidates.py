...
@dataclass
class Candidate:
    problem_id: str; name: str; scores: dict[str,float]
...
def write_candidate_problem(artefacts_dir: Path, candidates: list[Candidate]) -> Path:
    ...
    for i, c in enumerate(candidates, start=1):
        ...
        lines.append(""); lines.append(f"**Weighted score: {c.weighted_score():.2f}**"); lines.append("")
        ...
        if c.panel_picks:
            ...
            lines.append("")
        if c.reasoning:
            lines.append("**Reasoning:**"); lines.append(c.reasoning); lines.append("")
